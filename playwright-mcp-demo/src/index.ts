...
let dashboardIO: any = null;
const DASHBOARD_PORT = 3000;

// Dashboard test suite state
interface TestCase {
  testName: string;
  status: string;
  progress: number;
...
server.tool(
  ...
  "Start the live OWASP security testing dashboard on port 3000",
  {
  ...
  async ({ port = DASHBOARD_PORT }) => {
    ...
    try {
      ...
      dashboardIO.on('connection', (socket: any) => {
...
server.tool(
  ...
  "Get the current status of the live dashboard",
  {},
  async () => {
...
server.tool(
  "run_dashboard_scan",
  "Start a comprehensive OWASP security scan through the dashboard (results will be visible in dashboard UI)",
  {
...
async function runComprehensiveDashboardScan(targetUrl: string, socket: any) {
  try {
    ...
    const criticalCount = testSuite.reduce((sum, test) => sum + test.vulnerabilities.filter((v: any) => v.severity === 'CRITICAL').length, 0);
    const highCount = testSuite.reduce((sum, test) => sum + test.vulnerabilities.filter((v: any) => v.severity === 'HIGH').length, 0);
    const mediumCount = testSuite.reduce((sum, test) => sum + test.vulnerabilities.filter((v: any) => v.severity === 'MEDIUM').length, 0);
    const lowCount = testSuite.reduce((sum, test) => sum + test.vulnerabilities.filter((v: any) => v.severity === 'LOW').length, 0);
...
async function testBrokenAccessControlDashboard(url: string, socket: any) {
  const testIndex = 0;
...
async function testCryptographicFailuresDashboard(url: string, socket: any) {
  const testIndex = 1;
  testSuite[testIndex].status = 'running';
  testSuite[testIndex].startTime = new Date();
  socket.emit('testUpdate', testSuite[testIndex]);
...
async function testSecurityMisconfigurationDashboard(url: string, socket: any) {
  const testIndex = 4;
  testSuite[testIndex].status = 'running';
  testSuite[testIndex].startTime = new Date();
  socket.emit('testUpdate', testSuite[testIndex]);
...
async function runMockTestDashboard(testIndex: number, testName: string, socket: any) {
  testSuite[testIndex].status = 'running';
  testSuite[testIndex].startTime = new Date();
  socket.emit('testUpdate', testSuite[testIndex]);
