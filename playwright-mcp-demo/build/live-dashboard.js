...
async function testBrokenAccessControl(url, socket) {
    const testIndex = 0;
...
async function testCryptographicFailures(url, socket) {
    const testIndex = 1;
    testSuite[testIndex].status = 'running';
    testSuite[testIndex].startTime = new Date();
    socket.emit('testUpdate', testSuite[testIndex]);
...
async function testSecurityMisconfiguration(url, socket) {
    const testIndex = 4;
    testSuite[testIndex].status = 'running';
    testSuite[testIndex].startTime = new Date();
    socket.emit('testUpdate', testSuite[testIndex]);
...
async function runMockTest(testIndex, testName, socket) {
    testSuite[testIndex].status = 'running';
    testSuite[testIndex].startTime = new Date();
    socket.emit('testUpdate', testSuite[testIndex]);
...
async function runComprehensiveScan(targetUrl, socket) {
